    ...
    <div class="detailsList">
        ...
        <div v-if="roomDetails">
            <div v-if="roomDetails.length">
                <RoomDetailsSpecific @setActiveEvent="setActiveEvent" v-for="details in roomDetails" :details="details" :activeEvent="activeEvent" :key="details.id"/>
            </div>
            <div v-else-if="roomDetails.length==0">The room is currently empty.</div>
        </div>
...
export default{
    ...
    created(){
        axios.get(this.link + this.$route.params.id, { headers: { 'authentication': this.token } }).then((response) => {
